...
export default function OrthoModelStep({
  onSelect,
  onBack,
}: {
  ...
  const selectedModel = TRADITIONAL_MODELS.find((m) => m.id === selectedModelId);
  ...
  return (
    <div>
      ...
      <p className="text-sm text-slate-500 text-center mb-6">
        Selecione o tipo de aparelho ortodôntico
      </p>

      {/* Model cards */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {TRADITIONAL_MODELS.map((model) => {
      ...
      {/* Upsells (only shown when a model is selected) */}
      {selectedModel && (
        <div className="max-w-2xl mx-auto space-y-6 mb-8">
          ...
          {/* Summary bar */}
          <div className="bg-cyan-50 rounded-xl border border-cyan-200 p-5 flex items-center justify-between">
            <div>
            ...
            <button
              ...
              className="flex items-center gap-2 bg-cyan-600 hover:bg-cyan-700 text-white px-5 py-2.5 rounded-lg text-sm font-medium transition"
            >
              Continuar
